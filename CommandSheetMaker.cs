...
namespace RAA_Level2
{
    ...
    public class CommandSheetMaker : IExternalCommand
    {
          ...
          ElementSet elements)
        {
            ...
            // step 2: open form
            MyFormSheetMaker currentForm = new MyFormSheetMaker()
            {
                Width = 500,
                MinWidth = 500,
                Height = 380,
            ...
            foreach (string[] currentLevelData in levelData)
            {
                ...
                // create sheets
                /*
                if (currentForm.CreateFloorPlanSheets() == true)
                {
                    ViewFamilyType sheetVFT = GetViewFamilyTypeByName(doc, "Sheet Type", ViewFamily.FloorPlan);
                    ViewPlan currentPlan = ViewPlan.Create(doc, planVFT.Id, currentLevel.Id);
                }
                if (currentForm.CreateCeilingPlanSheets() == true)
                {
                    ViewFamilyType ceilingPlanVFT = GetViewFamilyTypeByName(doc, "Ceiling Plan", ViewFamily.CeilingPlan);
                    ViewPlan currentCeilingPlan = ViewPlan.Create(doc, ceilingPlanVFT.Id, currentLevel.Id);
